...
| Accents      | `cherry` / `cherry-deep` (fruit/brand) · `survey` (place/altitude) · `brass` / `brass-deep` (sweet)                              | Accent **text** = `cherry-deep` / `survey` / `brass-deep`; `brass` is fills/decoration only (fails AA as text); fills carry white text. |
